...
Problemi:
1) Il bridge non muove veramente gli NFT, ma invia i vecchi NFT ad un address vuoto, "bruciandoli" e poi li clone in una nuova chain.
I contratti rimangono attivi
possiamo considerare i nuovi NFT creati come originali? o sono solo delle copie?
Se gli NFT vengono portati indietro, non vengono spostati all'interno del vecchio contratto, ma sono creati da 0 in un nuovo contratto.
Questo rende gli NFT "copie"?
Al momento la funzionalità ha senso solo con contratti che contengono 1 NFT, questo per mantenere la collezione intatta, è estendibile a tutta la collezione?
La migrazione passa attraverso un oracolo, il quale a sua volta effettua la chiamata rest alla nostra API wrappata, questo vuole dire che la api non è realmente de-centralizzata, è comunque accettabile?
Potenzialmente lo spostamento degli NFT tramite le API thentic era implementabile on-chain, abbiamo deciso di creare un endpoint ad-hoc lato server per risparmiare sulle gas fees, siamo sicuri che questa sia stata una buona scelta?